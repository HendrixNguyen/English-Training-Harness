...
1. Find or create today's manual run: `RUN=$(printf "%s\n" harness/ideas/$(date +%F)-run-* | grep -v "\*" | tail -1)` (use a glob, not `ls` — output-rewriting hooks such as rtk can mangle `ls`); if empty, `RUN=$(python3 tools/harness/cli.py new-run)`.
