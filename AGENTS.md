...
- Never merge to `main` by hand or by pushing — `/harness merge` is human-only. Agents merge into `main` only through a GitHub PR with every check green, and only two kinds (owner, 2026-09-26): harness-artifact PRs (every file under `harness/` or `.agents/`; the decide and review routines merge their own, every routine merges green ones at its start) and the daily code PR, which the 20:00 review run merges when every included plan is reviewed `pass`/`pass-with-bugs`, blocker-free and CI is green. Rules in `.agents/routines/README.md`.
...
python3 tools/harness/cli.py doctor                   # exit 1 if this machine or a tool adapter misses the toolchain
