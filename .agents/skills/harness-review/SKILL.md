...
4. Walk the diff against the plan tasks (code-review skill; typescript-review for Nuxt code). Walk the idea's Expected output against the result. Then review for **quality** — the role's scope list: design and boundaries, correctness on untried inputs, performance and resource use, conventions and idiom, error handling, test honesty, documentation accuracy, security.
