...
4. **Read, in this order, and no more than needed:** `harness/CODEMAP.md`; the spec sections relevant to the mode (see AGENTS.md *Reading the spec*: the 1st-thinking doc §1, §5, §7 plus the backend spec §6/§8 and frontend spec §5/§7 for features; 1st-thinking §2–§4, §6, §7 plus backend spec §6 for mvp); the last two `_run.md` files; if a `remembering-conversations` skill is available, query it for prior decisions about this project.
