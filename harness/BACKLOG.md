# Backlog — ideas the owner has parked

Not a queue. Nothing here is `proposed`, so the evaluator never sees it and `next --stage evaluate` never lists it. Each entry names the **trigger** that un-parks it; the 02:00 ideate routine reads this file and, when a trigger is met, re-files the entry as a `proposed` idea in that day's run (`new-idea --source human`, body copied from here). Owner adds or removes entries by hand — this file is not frontmatter and `cli.py` does not manage it.
