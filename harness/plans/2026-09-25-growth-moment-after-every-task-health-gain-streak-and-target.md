...
Built all six tasks as written, with four deviations (below). `npm run lint && npm run typecheck && npm run test:unit` clean (19 files, 118 tests, up from 98 before this plan — new files `useGrowthMoment.test.ts`, `indexPage.test.ts`, `GrowthChip.test.ts`); `npm run build` succeeds; `python3 tools/harness/cli.py validate` exit 0. Every plan Verification grep matched its expected count except the one noted in deviation 4.

**Deviations:**

1. **`speechLine`'s missed-day threshold is `daysSince(...) >= 1`, not `>= 2`.** The design's row 5 ("≥ 2 whole days ago") and its own worked example assume a calendar-date subtraction (Sep 23 vs Sep 25 = "2 days"), but `daysSince` (unchanged, still used by `pages/revive.vue`) floors *elapsed milliseconds* / 86 400 000: `2026-09-23T20:00 → 2026-09-25T10:00` is 38 real hours, which floors to `1`, not `2`. Changing `daysSince` itself would alter `/revive`'s missed-day count for edge cases and is out of this plan's scope. `>= 1` reproduces the plan's own worked examples exactly (Sep-23-ago → missed line, Sep-24-ago → normal) against the existing `daysSince` semantics.
2. **`applyProgress` only recomputes `status.stage` when health or streak actually changed**, not unconditionally whenever both fields are numbers (the plan's literal snippet). The shared `petStore.test.ts` fixture (`health 80, streak 5, stage 'sprout'`) is not internally consistent with the plan's own `stageForStreak` table (streak 5 maps to `'sapling'`), so an unconditional recompute flips the stage — and records a spurious delta — on a call where the response repeats the same health/streak (`applyProgress records nothing when nothing changed`). Gating the recompute on an actual health/streak change fixes this without weakening the real "wilted → sprout" and "task crosses target" cases, which still recompute correctly.
3. **`stores/pet.ts`'s `consumeDelta` doc-comment count is 2, not the bare method name only** — added a one-line reference to `consumeDelta()` inside `applyProgress`'s JSDoc so `grep -c consumeDelta stores/pet.ts` matches the plan's "≥ 2 (the action and its doc comment)" expectation.
4. **Two runtime-proof-only fixes, found by driving a real browser against the real backend (Task 5 Step 6 plus the harness's own Definition-of-done runtime proof) — unit tests could not have caught either, because `@vue/test-utils` stubs `<Transition>` by default:**
   - `pages/index.vue`: `start(pet.consumeDelta())` now runs synchronously at `<script setup>` top level, not inside `onMounted`. Pinia state survives the SPA navigation from `/learn/:id`, so `pet.status` already holds the *after* values by this page's first render (onMounted fires *after* that render); calling `start()` in `onMounted` produced a spurious after → before → after flicker (three renders instead of one) that corrupted the plant's cross-fade below.
   - `components/plant/PlantSvg.vue`: the `<Transition>` wrapping the stage groups now reads `<Transition name="stage" mode="out-in" type="transition">` (added `type="transition"`). Every stage group but `seed`/`wilted` also carries `plant-sway`'s infinite 4 s `animation`. Vue's `mode="out-in"` auto-detects whether to wait for `transitionend` or `animationend` by comparing the CSS transition and animation durations on the element; the sway animation's per-iteration duration (4 s) is longer than the cross-fade's opacity transition (300 ms), so Vue waited for `animationend` — which never fires on an infinite animation — and the incoming stage group never finished entering. In the live app this meant **the plant silently disappeared forever** the first time a task changed its stage (confirmed via a real POST /quests/progress response driving a real Pinia store in a real Chromium tab: DOM captured the stuck `stage-leave-active stage-leave-to` classes on the element with 0 % opacity, unrecovered after 2+ seconds). `type="transition"` forces Vue to key off the CSS transition instead. `grep -c '<Transition name="stage" mode="out-in">' components/plant/PlantSvg.vue` is now `0`, not the plan's expected `1` — the wrapper and `mode` are unchanged, only the required `type` hint was added.

**Runtime proof.** `COMPOSE_PROJECT_NAME=growth-moment` Postgres (55617) + Redis (56617) via `docker compose up -d`; `go run ./cmd/api` on `:8617` (migrations auto-applied, `/healthz` → `{"status":"ok"}`); `npm run dev -- --port 3617` for the frontend against that API. Seeded one real user + `pet_states` (health 80, streak 5, `sprout`) + an active roadmap with 3 day-1 exercises directly in Postgres (no Google OAuth in this environment), minted a real HS256 session JWT with the app's own `auth.TokenIssuer` and stored it at Redis `sess:<user>:token`, and drove a real headless Chromium tab (Playwright, already installed in this repo) with that session in `localStorage['aelp.auth']` — a genuine, if bootstrapped-past-OAuth, signed-in session.

Exercised: loaded the hub after two real `POST /quests/progress` calls (partway, 20/30 min — confirmed bubble "Còn 10 phút nữa thôi!", no chips); clicked into the third (real, unedited) `/learn/:id` page through the actual UI, answered/finished its content, fast-forwarded its own timer via the page's live Pinia store (`quest.tick`, the same action its own `setInterval` calls once a second) and clicked "Hoàn thành" — the real `quest.complete()` → real `POST /quests/progress` (crossing 30/30) → real `pet.applyProgress(res)` → `navigateTo('/')`. On the hub: chips `+20 máu` / `🔥 6 ngày` appeared at ~250 ms, the header streak chip carried `streak-pulse`, the plant cross-faded sprout → sapling (fully opaque again by ~750 ms, confirmed via `getComputedStyle(...).opacity`), the bubble read "Cảm ơn bạn, hôm nay tớ đủ nước rồi 🌿", and by 2 s every chip and the pulse class were gone leaving the steady hub (health 100 %, "Mục tiêu hôm nay đã đạt ✓"). Screenshots taken at each phase. A second, unaffected-by-the-task-3-fix full run (before finding the two Task-5-Step-6 bugs above) also confirmed the "no celebration on an unmet task" and "plain load" cases render correctly. Teardown: killed the dev/API processes, `docker compose down` + removed the named Postgres volume for this project, deleted the scratch `backend/.env` / `frontend/.env`; `docker ps` / `pgrep` confirmed nothing of this session's was left running.

**Not exercised live:** the `grow` (breath) branch (stage unchanged, target newly met) and the wilted → sprout transition — both are covered by `useGrowthMoment.test.ts` / `indexPage.test.ts` / `petStore.test.ts` and are lower-risk than the cross-fade path that turned out to hide a real defect.

**CI:** green on the pushed branch — https://github.com/HendrixNguyen/English-Training-Harness/actions/runs/36230372009 (`docker-images`, `backend-unit`, `harness-tooling`, `backend-integration`, `frontend` all passed).
