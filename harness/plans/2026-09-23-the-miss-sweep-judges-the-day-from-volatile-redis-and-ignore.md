...
- **Residual race, documented:** a sweep judging D−1 and an `OnTargetMet` for D within the same second (a task straddling midnight reported at 00:00:0x while the tick runs) can land in either order; miss-then-met gives 90/1, met-then-miss gives 70/0 — the difference is one streak day and 20 health, in a sub-second window, and the streak reset is spec-conformant either way. Per-day rows would remove it; not worth a table.
  _Resolved 2026-09-24 by harness/plans/2026-09-24-get-quests-daily-still-reads-is-target-met-from-the-volatile.md: SaveTargetMet computes in SQL on the live row; the third outcome (100/1, the erased penalty) can no longer occur._
...
Built and executed all 8 tasks task-by-task with TDD (failing test → implement → pass → commit), then a full verification pass, live proofs against real Postgres/Redis, and a green CI run. Branch: `harness/2026-09-23-medium-the-miss-sweep-judges-the-day-from-volatile-redis-and-ignore`. Worktree: `.worktrees/the-miss-sweep-judges-the-day-from-volatile-redis-and-ignore`.

**Deviations from the plan, with reasons:**

1. **Task 7's lost-counter monotonicity check moved to the end of `TestIntegrationDailyAndProgressAgainstRealServices`, using the free `suite.Tasks[2]`, instead of right after the first `(30, true)` `daily_progress` assertion as the plan's snippet shows.** Inserting it at the plan's literal location changes the Redis counter and `daily_progress` state that the test's own later "Review reproduction 1/2" sections assert are *untouched* (`assertUntouched` expects `total==1800`, `daily_progress==(30,true)`) — running it there would have broken those assertions. Placed at the end instead, after accumulated reaches 1860s/31m; the expected minutes after +600 then a lost counter is **41** (not the plan's illustrative 40, which assumed starting from 1800s). The behaviour under test — a lost Redis key cannot lower `minutes_spent` or unset `is_target_met` — is unchanged.
2. **A gap the plan's Task 1 file list didn't cover:** `backend/internal/store/integration_test.go` (gated on `TEST_DATABASE_URL`, not listed in the plan's *File structure* table) independently hardcodes the applied-migration list (`{"0001_init", "0002_google_sync"}`) and count (`const versions = 2`) in `TestIntegrationMigrateAppliesToAnEmptyDatabaseAndIsIdempotent` and `TestIntegrationConcurrentMigrateDoesNotRace`. Both went red under `make test-integration` once `0003_pet_verdict_dates` existed. Fixed both to expect three versions (commit `store: update integration test expectations for migration 0003`) — this is why the branch has **9 commits, not 8**. I also tried adding `0003_pet_verdict_dates.down.sql` to the package's `reset()` helper for symmetry with 0001/0002, but reverted that: unlike 0001/0002's DDL, 0003's `ALTER TABLE` has no `IF EXISTS` on the table itself, so running it against an already-empty database (which `reset()` must tolerate — it runs both before and after every test in the file) fails with `relation "pet_states" does not exist`. 0001's `DROP TABLE IF EXISTS pet_states` already removes 0003's columns along with the table, so no down-migration call was needed there.

**Mutation table — every row run; two rows do not fail on the test named in the plan (behaviour is still pinned, just by a different test), reported as asked:**

- **"Sweep idempotent / concurrent" row, `PenaliseMiss` unconditional mutation:** does **not** turn `TestSweepIsIdempotentAcrossTicksAndHours` red. That test calls `Sweep` four times *sequentially* on one goroutine; `Service.Sweep` re-reads fresh state via `SweepCandidates` on every call, so its own `case c.State.JudgedThrough != nil && *c.State.JudgedThrough >= judged: continue` already prevents a second `PenaliseMiss` call once the first one lands — the fake's own idempotence predicate is never exercised in that path. `TestTwoConcurrentSweepsPenaliseOnce` **does** fail under the same mutation (sum 40, not 20), and the real SQL predicate is independently pinned by `TestIntegrationVerdictWritesAreConditional` §2 (8 concurrent `PenaliseMiss`, exactly 1 applies). So the property is covered, just not by the specific unit test the table names for the sequential half.
- **"`daily_progress` monotonic" row, fake-level `row.minutes = minutes` (drop `max`) mutation:** does **not** turn `TestTheHookFiresFromTheDurableFlagNotTheCounterEdge` red, because that test's lost-counter scenario re-adds the *same* 1800s both times, so the "new" minutes value (30) happens to equal the old one — no actual decrease occurs to expose the missing `max`. The real behaviour is pinned by the integration test's monotonicity block (deviation 1 above: minutes stays 41 after a smaller re-add), which does fail under the equivalent SQL mutation (`minutes_spent = EXCLUDED.minutes_spent`).
- **"A met day never shields an unjudged miss" row, `ApplyTargetMet` also advancing `JudgedThrough` mutation:** does **not** turn `TestSweepStillJudgesYesterdayWhenTodaysTargetWasMetFirst` red, because `OnTargetMet`'s write path (`Repo.SaveTargetMet` / the fake's `SaveTargetMet`) never persists `JudgedThrough` at all — it mirrors the real `saveTargetMetSQL`, whose column list has no `judged_through` — so a bug at the `ApplyTargetMet` level is absorbed before it reaches storage. The mutation **is** caught, by `TestApplyTargetMetStampsTheLocalDateItWasAppliedFor` (engine-level: asserts `JudgedThrough` stays nil after a success), just not by the sweep-level test the table names.
- Every other row (16 of 18, plus the 401-guard row and the pet-side-once-per-day row) failed exactly on the named test(s) when mutated, including the two rows gated on real Postgres (SQL predicates §1–3, SQL-miss-arithmetic-mirrors-`ApplyMiss` §4 via the `CASE WHEN … < 0` off-by-one). "Count is honest" (`penalised++` regardless of `applied`) failed `TestTwoConcurrentSweepsPenaliseOnce` in 7 of 8 repeated runs — the one pass is goroutine-scheduling variance in a genuinely concurrent test, not a defect in the assertion.

**Definition of done, all six:**
1. **Builds** — `go build ./...` clean. `gofmt -l` clean for every file this plan touches; two files fail gofmt on `main` untouched by this plan (`internal/quests/handler_test.go`, and `internal/quests/repo.go`'s pre-existing `Exercise.TaskType` comment alignment) — verified against `git show main:...` before leaving them.
2. **Whole suite**, clean shell: `env -u DATABASE_URL -u REDIS_URL -u TEST_DATABASE_URL -u TEST_REDIS_URL go test ./... -count=1 -race -timeout 300s` → `ok` for every package. `go test -race ./internal/pet/... ./internal/quests/...` → clean, no race reports. `make test-integration` (Postgres 5454 / Redis 6402, `COMPOSE_PROJECT_NAME=petjudge`) → `ok` for every package including `store`, `pet`, `quests`.
3. **Boots and serves a real request** — `go run ./cmd/api` against the same Postgres/Redis, applied `[0001_init 0002_google_sync 0003_pet_verdict_dates]` at boot, `GET /healthz` → `200 {"postgres":"ok","redis":"ok","status":"ok"}`, `GET /api/v1/pet/status` (no token) → `401 {"error":"unauthorized"}` (the new handler guard, exercised live). Shut down cleanly (`kill` both the `go run` wrapper and its compiled child under `/var/folders/.../exe/api`; confirmed via `pgrep`).
4. **Every documented command works as documented** — `go build ./...`, `go vet ./...`, `gofmt -l`, `make up`/`make test-integration`/`make down`, the CI-run check, all ran and behaved as the plan and `AGENTS.md` describe.
5. **CI green**: run [35851715543](https://github.com/HendrixNguyen/English-Training-Harness/actions/runs/35851715543) — `frontend`, `harness-tooling`, `backend-unit`, `backend-integration` all passed.
6. **Commands are safe** — the destructive `store` integration tests are gated on `TEST_DATABASE_URL`/`TEST_REDIS_URL`, never the production `DATABASE_URL`/`REDIS_URL`; verified `docker compose` used only the scratch `petjudge` project on non-default ports.

**Runtime proof (live, against real Postgres 5454 / Redis 6402):**
- Two `OnTargetMet` calls for `2026-09-22` leave `current_streak` at 1 and the second `SaveTargetMet` reports `applied=false` — `TestIntegrationVerdictWritesAreConditional` §1, PASS.
- 8 concurrent `PenaliseMiss` calls for the same judged day: exactly 1 reports `applied=true`, health drops by exactly 30 — same test §2, PASS.
- `daily_progress` stays monotonic (minutes 41, `is_target_met` true) after `DEL`ing the Redis counter mid-day and reporting a smaller duration — `TestIntegrationDailyAndProgressAgainstRealServices`'s new block, PASS.
- A zone that skips local midnight (America/Santiago, 2026-09-06: clocks jump 23:59:59 → 01:00:00) is judged at its 04Z tick (01:00 local): verified with a scratch program (`backend/cmd/liveproof-scratch`, written to the scratch dir, run via `go run ./cmd/liveproof-scratch` against the real stack, then deleted — never committed) seeding a real `pet_states` row with `timezone='America/Santiago'`, `judged_through='2026-09-04'`, calling the real `Service.Sweep` with the real `PgRepo`/`RedisCounter`/`RedisChallengeStore`: `penalised=1`, `health=70`, `judged_through=2026-09-05`. Output: `Sweep at 2026-09-06T04:00:00Z (America/Santiago local time 2026-09-06 01:00 -0300) penalised=1` / `post-sweep state: health=70 stage=sprout streak=0 judged_through=2026-09-05` / `LIVE PROOF PASSED`.

**Migration / spec DDL diff** — `grep -c 'ADD COLUMN judged_through DATE;' internal/store/migrations/0003_pet_verdict_dates.up.sql "../project-base/Adaptive English Learning Platform - Backend Technical Specification.md"` → `1` and `1`; byte-identical `ALTER TABLE` statement in both.

**`cmd/api/main.go`** — `git diff main..HEAD --stat -- backend/cmd/api/main.go` is empty; untouched.

**Cleanup** — `docker compose down -v` (containers, network and volume for `petjudge` removed), scratch `backend/.env` deleted, orphaned `exe/api` child process force-killed after the `go run` wrapper's kill didn't reap it, confirmed via `docker ps -a --filter name=petjudge` (empty) and `pgrep -fl "exe/api"` (empty). No pre-existing files or other executors' worktrees touched.
