...
| `POST /api/v1/pet/revive` §6.3 `{answers}` → `{revival_passed, pet_state}`; 409 `pet_not_wilted` | `usePetStore.revive` | `/revive` (7.5) | **live on `main`** |
...
- **Live vs stubbed at execution.** Live on `main`: `/login` (wrong shape, above), `/` progress + quests + plant hub, `/learn/:id`, `/roadmap` (via `day_number`), `/revive` (`/pet/status` and `/pet/revive` landed on `main` in `c33fb73` while this plan was written). Stubbed: `/onboarding` (quiz + assessment) behind `NUXT_PUBLIC_STUB_ONBOARDING`. Unwired: `/settings` buttons (notify, google).
