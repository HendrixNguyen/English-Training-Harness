...
**Applied immediately on the live database by hand (owner + orchestrator, 2026-09-25):** `ALTER TABLE … ENABLE ROW LEVEL SECURITY` on all eight tables (no policies → deny for `anon`/`authenticated`; `SET ROLE anon; SELECT count(*) FROM users` now returns 0 rows/permission denied, `/healthz` still 200). This is *not* in the migrations, so the next `CREATE TABLE` in a migration reopens the hole and the advisor flags it again.
