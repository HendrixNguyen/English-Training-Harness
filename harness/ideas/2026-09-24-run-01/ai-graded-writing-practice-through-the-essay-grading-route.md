...
_Evaluator, 2026-09-26 — **deferred** (not planned today)._ The `writing` practice variant belongs to the content schema on the typed-content branch (merges tonight); planning against unmerged code risks a conflict on `content.go`. Tomorrow's feature queue, backend plan first.
